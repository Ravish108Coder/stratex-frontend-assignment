...
export default function FavoriteMovies({isSortedByRating, setIsSortedByRating}) {
  const dispatch = useDispatch();
  const movies = useSelector((state) => state.movies).filter((item) => item.favorite === true);
  return (
    <div>
      <div className="p-5 px-10 flex items-center justify-between text-white">
        <button className=" border p-4 px-5 rounded-xl bg-green-700" >{movies.length + " favorite movies"}</button>
        <Link to={"/"}>
        <button className="border rounded-xl p-4 px-5 hover:bg-purple-600 hover:text-white">Go to Movies List</button>
        </Link>
      </div>
